...
6. 击杀敌方英雄获得职业对应积分与金币。
7. 牌堆取空进入本关下一轮（仍为本关时代牌池），继续打同一座敌塔。
8. 打爆敌方防御塔即过关，进入下一关；己方防御塔被打爆即失败。
...
- **同关多轮**：本关牌堆取空只推进轮次，卡池时代与关卡锁定，不会偷升到下一关（旧的 `ERA_UP_ROUNDS` 轮次升时代已移除）。
...
牌池时代与当前关卡锁定（`GameData.blended_deck_counts(era_index)`，以本关时代为主、低关旧卡少量混入）。同关内牌堆取空只发下一轮的本关牌；过关时牌堆与合成台剩牌直接丢弃，下一关从第 1 轮重新发牌。
...
击杀敌方英雄时，己方队伍增加该英雄职业对应的 `kill_score`，并获得等于该积分值乘时代倍率的金币；塔击杀给金币但不给积分。信息栏显示「第 N 关（时代名）· 第 R 轮 · 状态」。关卡推进只看是否打爆敌塔，积分仅用于本局结算与最高分。第 5 关（未来）为终点。

...
func _initialize() -> void:
	...
	_check(main.STARTING_COINS == 400, "开局金币常量应为 400")
	_check(main.coin_count == 400, "开局金币应为 400，实际 %d" % main.coin_count)
	_check(main.PLAYER_KILL_GOLD == 0, "玩家击杀金币应归零")
	_check(main.KILL_SCORE_MULT <= 0.25 + 0.0001, "击杀积分应弱化至 ≤0.25")

	# 击杀敌兵不给金币；非塔击杀给弱化积分
	var coins_before := main.coin_count
	var score_before := main.kill_score
	var enemy := _spawn_test_enemy()
	_check(enemy != null, "应能生成测试敌兵")
	if enemy != null:
		enemy.last_damage_source = "ally"
		enemy.score_awarded = false
		var raw_score := int(enemy.stats.get("kill_score", 0))
		main._on_unit_expired(enemy)
		_check(main.coin_count == coins_before, "击杀敌兵不应增加金币")
		var expected_score := score_before + maxi(0, int(round(float(raw_score) * main.KILL_SCORE_MULT)))
		_check(main.kill_score == expected_score, "击杀积分应按弱化系数累加，期望 %d 实际 %d" % [expected_score, main.kill_score])

	# 塔击杀：无金币、无击杀分
	coins_before = main.coin_count
	score_before = main.kill_score
	var tower_kill := _spawn_test_enemy()
	if tower_kill != null:
		tower_kill.last_damage_source = "tower"
		tower_kill.score_awarded = false
		main._on_unit_expired(tower_kill)
		_check(main.coin_count == coins_before, "塔击杀不应增加金币")
		_check(main.kill_score == score_before, "塔击杀不应增加击杀积分")

	# 速度公式：elapsed=target → 倍率 1.0；更快 → >1；更慢 → <1
	main.stage_time_limit = 180.0
	main.stage_elapsed = 180.0
	_check(is_equal_approx(main._stage_speed_mult(), 1.0), "用时=目标时速度倍率应为 1")
	main.stage_elapsed = 90.0
	_check(main._stage_speed_mult() > 1.0, "更快通关速度倍率应 >1")
	_check(main._stage_speed_mult() <= main.CLEAR_GOLD_SPEED_MAX + 0.0001, "速度倍率不应超过上限")
	main.stage_elapsed = 900.0
	_check(main._stage_speed_mult() >= main.CLEAR_GOLD_SPEED_MIN - 0.0001, "极慢通关速度倍率应夹在下限")

	# 过关发放金币 + 关卡分 + 速度分
	main._start_round(0)
	await process_frame
	main.stage_elapsed = 90.0
	main.stage_time_left = maxf(0.0, main.stage_time_limit - 90.0)
	main.stage_clear_economy_awarded = false
	coins_before = main.coin_count
	score_before = main.kill_score
	var expected_gold := main._compute_clear_gold()
	...
	_check(main.coin_count == coins_before + expected_gold, "过关后金币应增加公式值")
	_check(int(award.stage_pts) == int(scores.stage), "应发放关卡分")
	_check(int(award.speed_pts) == int(scores.speed), "应发放速度分")
	_check(
		main.kill_score == score_before + int(scores.stage) + int(scores.speed),
		"本局总分应累加关卡分+速度分"
	)
	# 幂等：同关不重复发
	var mid_coins := main.coin_count
	var mid_score := main.kill_score
	var again: Dictionary = main._award_stage_clear_economy()
	_check(int(again.gold) == 0, "同关重复过关结算不应再发金币")
	_check(main.coin_count == mid_coins and main.kill_score == mid_score, "幂等后金币/积分不变")
	...
	_check(main.coin_count > coins_before, "悬赏令应立即发放金币")
	_check(main._buff_active("bounty"), "悬赏令应进入 buff 计时")
	main.stage_elapsed = 180.0
	main.stage_time_limit = 180.0
	main.round_coin_mult = 1.0
	main.stage_clear_economy_awarded = false
	var with_bounty := main._compute_clear_gold()
	main.buff_timers.erase("bounty")
	var without_bounty := main._compute_clear_gold()
	_check(with_bounty > without_bounty, "悬赏令激活时应提升过关金币")

	# loot_boost 作用于过关金
	main.round_coin_mult = 1.3
	var boosted := main._compute_clear_gold()
	main.round_coin_mult = 1.0
	var normal := main._compute_clear_gold()
